...
if [[ -d "$STORAGE_ROOT" ]]; then
  ...
  if mountpoint -q "$STORAGE_ROOT"; then
    ...
    no "NOT a mountpoint — agreements would be written to the local disk"
  fi
  ...
  if [[ -f "$STORAGE_ROOT/.gtids-storage-root" ]]; then
    ...
    no "marker .gtids-storage-root missing — the API will refuse to start"
  fi
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  [[ "$(get NODE_ENV)" == "production" ]] && ok "NODE_ENV=production" || hmm "NODE_ENV=$(get NODE_ENV)"
  [[ "$(get PDF_RENDERER)" == "playwright" ]] && ok "PDF_RENDERER=playwright" \
    || no "PDF_RENDERER must be 'playwright' in production"
  [[ "$(get ESIGN_PROVIDER)" == "mock" ]] \
    && no "ESIGN_PROVIDER=mock — production will refuse to start (DEC-002 still open)" \
    || ok "ESIGN_PROVIDER=$(get ESIGN_PROVIDER)"
  [[ "$(get SMTP_TRANSPORT)" == "smtp" ]] && ok "SMTP_TRANSPORT=smtp" \
    || no "SMTP_TRANSPORT is '$(get SMTP_TRANSPORT)' — no mail would be sent"
  [[ -n "$(get SMTP_HOST)" ]] && ok "SMTP host configured" || no "SMTP_HOST is empty"
