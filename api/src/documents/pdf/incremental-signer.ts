...
/**
 * Employee approval (FR-012 / DEC-004): a visible attestation rendered into the
 * reserved widget by incremental update. Consumes no eSign transaction and leaves
 * the Agent signature intact.
 */
export function appendAttestation(
  buf: Buffer,
  opts: { fieldName: string; lines: string[]; fontObjectNumber: number },
): { buffer: Buffer; xobjectNumber: number } {
  const doc = parseDocument(buf);
  const field = findFieldByName(buf, doc.offsets, opts.fieldName);

  const rect = /\/Rect\s*\[\s*([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s*\]/.exec(field.body);
  if (!rect) throw new Error(`field "${opts.fieldName}" has no /Rect`);
  ...
  opts.lines.forEach((line, i) => {
    ...
    const colour = i === 0 ? '0.10 0.35 0.12' : '0.15 0.15 0.15';
    ops += `BT /Helv ${size} Tf ${colour} rg 5 ${y} Td (${escapePdfText(line)}) Tj ET\n`;
    y -= i === 0 ? 12 : 9;
  });
  ops += 'Q\n';
  ...
  const xobj =
    ...
    ` /Resources << /Font << /Helv ${opts.fontObjectNumber} 0 R >> /ProcSet [/PDF /Text] >>` +
  ...
  return {
    buffer: appendRevision(buf, doc, [
      { num: xobjNum, gen: 0, content: xobj },
      {
        num: field.num,
        gen: field.gen,
        content: injectIntoDict(field.body, `/AP << /N ${xobjNum} 0 R >>`),
      },
    ]),
